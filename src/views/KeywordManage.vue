<template>
  <el-card>
    ...
    <!-- 双保险：即使路由没拦住，也不让普通用户操作 -->
    <div v-if="!isAdmin" style="padding: 8px 0">
      <el-alert title="无权限" type="error" :closable="false" show-icon>
        <template #default>该功能仅管理员可使用。</template>
      </el-alert>
    </div>
    ...
    <div v-else>
      ...
      <!-- ✅ 第二行：关键词筛选 + 新增
       想要没选择报告类别前不能筛选关键词就在clearable后新增:disabled="!category"-->
